...
theme(
    ...
    xlim = (:auto, :auto),
    ylim = (:auto, :auto),
    xlab = L"m(\Xi_c^+K^-)\,\,[\mathrm{GeV}]",
    ylab = L"\mathrm{Candidates}",
)
#
#
const xth = 2.96
...
#
signalpdf0 = Normalized(abs2(SimpleBWg((m0 = 2.95, g = 0.01))) * phasespace, fitrange)
...
@time fit_summary = fit(Extended(NegativeLogLikelihood(model1, data)))
#
let bins = range(fitrange..., length = 80)
    Ns = scaletobinneddata(bins)
    stephist(data; bins, lc = :black, lab = "Data")
    for i = 1:5
        plot!(fit_summary.best_model[i], Ns, 300, fillto = 0.0, α = 0.4, lab = "Signal$(i)")
    end
