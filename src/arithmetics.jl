...
struct FProd{T1<:AbstractFunctionWithParameters,T2<:AbstractFunctionWithParameters} <:
...
updatevalueorflag(d::FProd, s::Symbol, isfree::Bool, v = getproperty(pars(d), s)) = FProd(
...
+(
    ...
    f2::AbstractFunctionWithParameters;
    p = (α1 = 1.0, α2 = 1.0),
) = FSum([f1, f2], p)
...
struct NegativeLogLikelihood{T<:AbstractFunctionWithParameters,D<:AbstractArray} <:
       AbstractFunctionWithParameters
    f::T
    data::D
    nagativepenatly::Float64
end
func(d::NegativeLogLikelihood, x::NumberOrTuple; p = freepars(d)) =
    -sum((v > 0) ? log(v) : d.nagativepenatly for v in d.f(d.data; p))
