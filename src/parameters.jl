...
subtract(t1::NamedTuple, ss::AbstractVector{Symbol}) =
    Base.structdiff(t1, NamedTuple{Tuple(ss)}(zeros(length(ss))))
subtract(t1::NamedTuple, ss::Tuple{Vararg{Symbol}}) =
    Base.structdiff(t1, NamedTuple{ss}(zeros(length(ss))))
...
#
selectintersect(p::NamedTuple, from_p::NamedTuple) =
    selectpars(from_p, intersect(keys(p), keys(from_p)))
...
updatevalueorflag(p::NamedTuple, s::Symbol, isfree::Bool, v = getproperty(pars(p), s)) =
    ...
    throw(
        ArgumentError(
            "Not able to fix, release parameters since parameters are held by a NamedTuple!",
        ),
    )
...
keys(ps::FlaggedNamedTuple, isfree::Bool) =
    isfree == false ? whichfixed(ps) : Base.diff_names(keys(allpars(ps)), whichfixed(ps))
pars(ps::FlaggedNamedTuple, isfree::Bool) = NamedTuple{keys(ps, isfree)}(allpars(ps))

getproperty(ps::FlaggedNamedTuple, s::Symbol) =
    hasproperty(freepars(ps), s) ? getproperty(freepars(ps), s) :
    getproperty(fixedpars(ps), s)


# inner working
updatednamedtuple(p::NamedTuple, s::Symbol, v) = NamedTuple{keys(p)}(merge(p, nt(s, v)))
mustinclude(whichfixed::Tuple{Vararg{Symbol}}, s) =
    s ∈ whichfixed ? whichfixed : (whichfixed..., s)
mustexclude(whichfixed::Tuple{Vararg{Symbol}}, s) =
    s ∈ whichfixed ? Base.diff_names(whichfixed, (s,)) : whichfixed
...
"""
function updatevalueorflag(
    p::FlaggedNamedTuple,
    s::Symbol,
    isfree::Bool,
    v = getproperty(pars(p), s),
)
...
merge(s1::FlaggedNamedTuple, s2::FlaggedNamedTuple) = FlaggedNamedTuple(
    merge(allpars(s1), allpars(s2)),
    (whichfixed(s1)..., whichfixed(s2)...),
)
