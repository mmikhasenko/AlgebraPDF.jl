...
@recipe function f(
    d::Union{AbstractPDF,FSumPDF},
    norm::T where {T<:Real} = 1.0,
    bins::Int = 100,
)
...
@recipe f(::Type{T}, d::T) where {T<:AbstractFunctionWithParameters} =
    x -> func(d, x; p = pars(d))
...
@recipe function f(
    ...
    @series begin
        ...
        bincontent =
            [sum(x -> l < x < r, data) for (l, r) in zip(binning[1:end-1], binning[2:end])]
        #
        x := centers
        y := bincontent
...
#
@recipe f(xv::ArrayOrRange, yv::ArrayOrRange, d::AbstractFunctionWithParameters) =
    (xv, yv, (x, y) -> func(d, (x, y)))
