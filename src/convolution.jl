...
conv_with_gauss(d::pdf,σ) = pdf(@. (e;p)->conv_with_gauss(e,x->d.f(x;p=p),σ); p=d.p, lims=lims(d))
...
conv_with_gauss_sampling(d::pdf,σ; Ns=10) =
    pdf((e;p)->conv_with_gauss_sampling(e, x->d.f(x; p=p), σ; Ns=Ns); p=d.p, lims=lims(d))
