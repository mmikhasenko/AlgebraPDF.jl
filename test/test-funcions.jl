...
@testset "FunctionWithParameters{NamedTuples}" begin
    d0 = FunctionWithParameters((x; p) -> p.a + cos(x) * p.b; p = (a = 2, b = 1))
    #
...
@testset "FunctionWithParameters{FlaggedNamedTuple}" begin
    m0 = FunctionWithParameters((x; p) -> p.a + cos(x) * p.b; p = Ext(a = 2, b = 1))
    #
    m1 = updatepar(m0, :a, 4)
...
@testset "FLog" begin
    m0 = FunctionWithParameters((x; p) -> exp(p.b * x); p = Ext(b = 2))
    #
    am0 = log(m0)
    @test am0(1) == 2
...
@testset "FSum and FProd" begin
    a1 = FunctionWithParameters((x; p) -> p.a + cos(x) * p.b; p = Ext(a = 2, b = 1))
    a2 = FunctionWithParameters((x; p) -> p.a + sin(x) * p.b; p = Ext(a = 2, b = 1))
    #
    sum1 = a1 + a2
